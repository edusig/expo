...
static NSString * const ABI42_0_0EXUpdatesExpoAssetBaseUrl = @"https://d1wp6m56sqw74a.cloudfront.net/~assets/";
static NSString * const ABI42_0_0EXUpdatesExpoIoDomain = @"expo.io";
static NSString * const ABI42_0_0EXUpdatesExpHostDomain = @"exp.host";
static NSString * const ABI42_0_0EXUpdatesExpoTestDomain = @"expo.test";
                                     ...
                                     database:(ABI42_0_0EXUpdatesDatabase *)database
{
  ...
  if (manifest.isUsingDeveloperTool) {
    ...
    update.commitTime = [ABI42_0_0RCTConvert NSDate:commitTimeString];
  }

  if (manifest.isDevelopmentMode) {
    update.isDevelopmentMode = YES;
    update.status = ABI42_0_0EXUpdatesUpdateStatusDevelopment;
  } else {
...
+ (NSURL *)bundledAssetBaseUrlWithManifest:(ABI42_0_0EXManifestsLegacyManifest *)manifest config:(ABI42_0_0EXUpdatesConfig *)config
{
  ...
  if (!host ||
      [host containsString:ABI42_0_0EXUpdatesExpoIoDomain] ||
      [host containsString:ABI42_0_0EXUpdatesExpHostDomain] ||
      [host containsString:ABI42_0_0EXUpdatesExpoTestDomain]) {
    return [NSURL URLWithString:ABI42_0_0EXUpdatesExpoAssetBaseUrl];
  } else {
